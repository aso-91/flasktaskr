...
@app.route('/add/', methods=['POST'])   # we used POST so we can add into our db.
@login_required
def new_task():
    g.db = connect_db()
...
@app.route('/complete/<int:task_id>/')  # task_id is filled in the function from tasks.html line 64 but as str
@login_required
